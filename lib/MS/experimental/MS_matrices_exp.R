...
b.LMatrix<-NULL
embeds<-embedD1:(embedD1+embedD2-1)
for(i in 1:embedD2){
  b.L<-NULL
  b.L1 <- rep(1,embeds[i])
  b.L2 <- c(1:embeds[i])*theTau*deltaT-mean(c(1:embeds[i])*theTau*deltaT)
  b.L3 <-  (b.L2^2)/2
  b.L<-cbind(b.L1,b.L2,b.L3)
  forMat<-matrix(0,1,embedD2); forMat[i]<-1
  b.LMatrix<-rbind(b.LMatrix, forMat%x%b.L)
}

embedD<-nrow(b.LMatrix)
manifestVars <- NULL
