...
numIndicators<-1
nLatentVars<-embedD1+6


DLO1.l<-c("X", "dX", "d2X")
DLO2.l<-c("Xm", "dXm", "d2Xm")
node.l<-paste0("t", 1:embedD1)

# lV.l<-c(rep(DLO1.l, embedD2), DLO2.l)

lV.l<-c(DLO1.l, node.l,DLO2.l)
...
LMatrix<-cbind(matrix(0, embedD, 3), matrix(1, embedD2, 1)%x%diag(embedD1), a.LMatrix)
...
# A matrix ----------------------------------------------------------------
AA<-matrix(0,nLatentVars,nLatentVars)
dimnames(AA)<-list( lV.l, lV.l)
...
Am.labels<-AA
Am.labels[AA==2]<-c("eta","zeta")
Am.labels[AA==3]<-c("etaM","zetaM")
Am.labels[AA<=1]<-NA

Am.sv<-AA
Am.sv[AA==1]<-b.LMatrix
Am.sv[AA==2]<- -0.2
Am.sv[AA==3]<- -0.02
...
Am.lb<-AA; Am.lb[T]<-NA
Am.ub<-Am.lb
Am.ub[AA==2][1]<- -0.00001
Am.ub[AA==3][1]<- -0.00001
...
# Am[seq(3, 3*(embedD2),3)-2, 3*(embedD2+1)-2]<-3
# Am[seq(3, 3*(embedD2),3),3*(embedD2+1)-1]<-3
# Am[seq(3, 3*(embedD2),3),3*(embedD2+1)-2]<-3
...
# Am.ub<-ifelse(Am.ub==0, NA, 0.0001)
# 


#S
SS<-diag(nLatentVars)
dimnames(SS)<-list(lV.l,lV.l)
SS[node.l,node.l]<-0
SS["dX","X"]<-2
SS["dXm","Xm"]<-3

Sm.labels<-SS
Sm.labels[Sm.labels==1]<-c(DLO1.l, DLO2.l)
Sm.labels[Sm.labels==2]<-"rX_dX"
Sm.labels[Sm.labels==3]<-"rXm_dXm"
Sm.labels[Sm.labels==0]<-NA
...
Sm.lb<-SS; Sm.lb[T]<-NA; Sm.ub<-Sm.lb
Sm.lb[SS==1]<-0.0000001
Sm.lb[SS>1]<- -1
Sm.ub[SS>1]<- 1
