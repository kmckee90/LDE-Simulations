...
DLOmodel <- function(t, prevState, parms) {
  ...
  with(list(parms, states, dx),
      ...
      states[,1] <- dx[,2]
      states[,2] <- rowSums(parms[[2]]%*%(dx*parms[[1]]))
      ...
      res<-vals
        # 
  #     for(i in 1:(nVars*2)){
  #       eval(parse(text=paste(varNames[i],"<-",vals[i],sep="")))
  #     }
  #     res<-eval(parse(text=paste(paste("c(", paste(varNames, collapse=","), ")"))))
      list(res)
       }
   )
  }
