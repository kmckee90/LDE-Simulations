...
# ---------------------------------------------------------------------
MS_genData<-function(N=100, SNR=2, numVars, parameters, initialConditions,
...
out.dat<-out1$x+out1$z

tSD <- sd(out.dat)
tESD <- 1 / tSNR

tData <- out.dat/tSD + rnorm(N, mean=0, sd=tESD)
...
if(genPlotting){plot(tData, type="l")}
