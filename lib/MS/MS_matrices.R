...
# L matrix ----------------------------------------------------------------
a.L1 <- rep(1,defaults$embedD) 
a.L2 <- c(1:(defaults$embedD))*defaults$theTau*defaults$deltaT-mean(c(1:(defaults$embedD))*defaults$theTau*defaults$deltaT)
a.L3 <-  (a.L2^2)/2
# a.L2 <- a.L2 /defaults$embedD1#*1.00726

b.L1 <- rep(1,defaults$embedD1) 
...
A.l<-c("etaX1", "zetaX1", "gamma", "etaXM1", "zetaXM1")
...
Am.sv[grepl('etaXM1', Am.labels)]<- -0.02
